# Variables d'environnement — Tolkee
...
SENTRY_ORG=tolkee                                   # slug de l'organisation Sentry
SENTRY_PROJECT=tolkee-slug                          # slug du projet Sentry
...
INTERNAL_PIPELINE_SECRET=hex(32 octets)             # secret partagé interne qui protège /api/transcribe et /api/analyze (routes service key, bypass RLS, appelées QUE par nos webhooks). Vérifie l'en-tête x-tolkee-internal en temps constant — cf. lib/internal-auth.ts (issue #2). FAIL-CLOSED : si absente, les deux routes renvoient 401 → pipeline cassé. SERVER-ONLY / Vercel (Production + Preview + Development). Générer avec `openssl rand -hex 32`.
